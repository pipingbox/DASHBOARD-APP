 ...
 *
 * DEPRECATED / NO-OP: profile_completion is backend-controlled and must not be
 * updated from the frontend. Onboarding completion now runs through the
 * complete-onboarding Edge Function. Keeping the signature for compatibility
 * with existing call sites.
 */
export async function recalculateAndSaveProfileCompletion(_userId: string): Promise<void> {
  // No-op: frontend no longer writes profile_completion.
