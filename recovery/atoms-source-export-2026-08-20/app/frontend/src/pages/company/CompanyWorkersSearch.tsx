...
export default function CompanyWorkersSearch() {
  ...
  const fetchWorkers = useCallback(async (applyFilters = true) => {
    ...
    try {
      ...
      // account_type = 'worker' OR role = 'worker' OR role = 'user' OR marketplace_ready = true
      // This captures all worker-type profiles AND any profile marked marketplace-ready;
      // visibility filtering is done client-side
      const { data: profiles, error: profilesError } = await supabase
        .from(TABLES.profiles)
        .select('*')
        .or('account_type.eq.worker,role.eq.worker,role.eq.user,marketplace_ready.eq.true');

      console.log('[WorkerSearch] Raw query result:', profiles);
      console.log('[WorkerSearch] Profiles fetched count:', profiles?.length ?? 0);

      // DIAGNOSTIC: Search specifically for "Pedro" or "Crespo" to check if RLS or OR filter is the issue
      const { data: pedroCheck, error: pedroErr } = await supabase
        .from(TABLES.profiles)
        .select('user_id, full_name, role, account_type, profile_visibility, marketplace_ready, onboarding_status, cv_visible')
        .or('full_name.ilike.%Pedro%,full_name.ilike.%Crespo%');
      console.log('[WorkerSearch] PEDRO DIAGNOSTIC - Direct name search:', pedroCheck, pedroErr);
      if (pedroCheck && pedroCheck.length > 0) {
        pedroCheck.forEach((p: Record<string, unknown>) => {
          const matchesORFilter = p.account_type === 'worker' || p.role === 'worker' || p.role === 'user' || p.marketplace_ready === true;
          console.log(`[WorkerSearch] PEDRO FOUND: ${p.full_name} | user_id=${p.user_id} | role=${p.role} | account_type=${p.account_type} | marketplace_ready=${p.marketplace_ready} | onboarding_status=${p.onboarding_status} | MATCHES_OR_FILTER=${matchesORFilter}`);
          if (!matchesORFilter) {
            console.warn(`[WorkerSearch] ⚠️ PEDRO DOES NOT MATCH OR FILTER! His role="${p.role}", account_type="${p.account_type}", marketplace_ready=${p.marketplace_ready}. Fix: set marketplace_ready=true or account_type='worker' in Supabase.`);
          }
        });
      } else {
        console.warn('[WorkerSearch] ⚠️ PEDRO NOT FOUND by name search. Either RLS blocks access or name is different in DB.');
      }
      ...
      if (!profiles || profiles.length === 0) {
        ...
        // Debug: fetch ALL profiles to check what exists and why none match the filter
        const { data: allProfiles, error: allErr } = await supabase
          .from(TABLES.profiles)
          .select('user_id, role, account_type, full_name, title, cv_visible, profile_visibility, marketplace_ready, onboarding_status')
          .limit(20);
        console.log('[WorkerSearch] DEBUG - First 20 profiles (any role/type):', allProfiles, allErr);
        if (allProfiles) {
          allProfiles.forEach((p: Record<string, unknown>) => {
            console.log(`[WorkerSearch] DEBUG PROFILE: ${p.full_name} | role=${p.role} | account_type=${p.account_type} | visibility=${p.profile_visibility} | marketplace_ready=${p.marketplace_ready} | cv_visible=${p.cv_visible}`);
          });
        }
        setWorkers([]);
        setLoading(false);
      ...
      // Show first 10 raw profiles in console for debug with key visibility fields
      console.log('[WorkerSearch] DEBUG - First 10 raw profiles:', profiles.slice(0, 10));
      profiles.slice(0, 10).forEach((p: Record<string, unknown>) => {
        console.log(`[WorkerSearch] PROFILE: ${p.full_name} | user_id=${p.user_id} | role=${p.role} | account_type=${p.account_type} | profile_visibility=${p.profile_visibility} | marketplace_ready=${p.marketplace_ready} | cv_visible=${p.cv_visible} | onboarding_status=${p.onboarding_status}`);
      });

      // MARKETPLACE FILTER: Show worker profiles that are visible to companies
      // Rules:
      // - Must have full_name (at minimum)
      // - marketplace_ready=true ALWAYS overrides profile_visibility (shown regardless)
      // - If NOT marketplace_ready: only hide if profile_visibility is EXPLICITLY 'private'
      // - If profile_visibility is null (legacy/never set), treat as visible
      const visibleProfiles = (profiles as WorkerProfile[]).filter((p) => {
        const hasName = !!p.full_name;
        // marketplace_ready=true overrides all visibility restrictions
        const isMarketplaceReady = p.marketplace_ready === true;
        const isExplicitlyPrivate = p.profile_visibility === 'private';
        // Show if: has name AND (marketplace_ready OR not explicitly private)
        const isVisible = isMarketplaceReady || !isExplicitlyPrivate;
        const show = hasName && isVisible;
        if (!show) {
          console.log('[WorkerSearch] EXCLUDED:', p.user_id, p.full_name, {
            reason: !hasName ? 'NO_NAME' : 'PRIVATE_AND_NOT_MARKETPLACE_READY',
            profile_visibility: p.profile_visibility,
            marketplace_ready: p.marketplace_ready,
          ...
          console.log('[WorkerSearch] INCLUDED:', p.user_id, p.full_name, {
            profile_visibility: p.profile_visibility,
            marketplace_ready: p.marketplace_ready,
            override: isMarketplaceReady && isExplicitlyPrivate ? 'MARKETPLACE_OVERRIDES_PRIVATE' : 'NORMAL',
          });
        }
        return show;
      });

      console.log('[WorkerSearch] Visible profiles:', visibleProfiles.length);
      console.log('[WorkerSearch] Excluded (private/no name):', profiles.length - visibleProfiles.length);

      if (visibleProfiles.length === 0) {
        // No visible profiles found
        console.warn('[WorkerSearch] No visible profiles found.');
        setWorkers([]);
        setLoading(false);
        return;
      }
    ...
    <div className="space-y-6">
      ...
      {!loading && !error && searched && workers.length === 0 && (
        <div className="border border-zinc-800/80 bg-[#0d0d0d] rounded-sm p-12 text-center space-y-3">
          ...
          <p className="text-sm text-zinc-400">{t('companyWorkerSearch.noWorkersFound', 'No workers found. Try changing your filters.')}</p>
          <p className="text-[11px] text-zinc-600">{t('companyWorkerSearch.checkConsole', 'Check browser console for debug information.')}</p>
        </div>
      )}
...
function WorkerCard({ worker, onView }: { worker: WorkerWithCounts; onView: () => void }) {
  const { t } = useTranslation();
  const availabilityBadge = getAvailabilityBadge(worker, t);
    ...
    <div className="border border-zinc-800/80 bg-[#0d0d0d] rounded-sm p-5 space-y-4 hover:border-zinc-700 transition">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4 className="text-sm font-semibold text-zinc-100 truncate">{worker.full_name || worker.username || t('companyWorkerSearch.worker', 'Worker')}</h4>
          <p className="text-xs text-zinc-400 truncate">{worker.title || t('companyWorkerSearch.positionNotSpecified', 'Position not specified')}</p>
          {worker.company && (
      ...
      <div className="flex flex-wrap gap-2 text-[11px]">
        {worker.location ? (
          ...
          <span className="flex items-center gap-1 text-zinc-600">
            <MapPin className="h-3 w-3" />
            {t('companyWorkerSearch.locationNotSpecified', 'Location not specified')}
          </span>
        )}
      ...
      {worker.skills && worker.skills.length > 0 ? (
        ...
        <div className="flex flex-wrap gap-1.5">
          <span className="rounded-sm bg-zinc-800/50 px-2 py-0.5 text-[10px] text-zinc-600">
            {t('companyWorkerSearch.noSkills', 'No skills listed')}
          </span>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-1.5">
        {availabilityBadge ? (
          ...
          <span className="inline-flex items-center gap-1 rounded-sm bg-zinc-800/50 border border-zinc-700/30 px-2 py-0.5 text-[10px] text-zinc-500">
            {t('companyWorkerSearch.availabilityNotSpecified', 'Availability not specified')}
          </span>
        )}
        {worker.willing_to_travel && (
          <span className="inline-flex items-center gap-1 rounded-sm bg-blue-950/50 border border-blue-800/30 px-2 py-0.5 text-[10px] text-blue-300">
            ...
            {t('companyWorkerSearch.travel', 'Travel')}
          </span>
        )}
        {worker.willing_to_relocate && (
          <span className="inline-flex items-center gap-1 rounded-sm bg-purple-950/50 border border-purple-800/30 px-2 py-0.5 text-[10px] text-purple-300">
            ...
            {t('companyWorkerSearch.relocate', 'Relocate')}
          </span>
        )}
      </div>
      ...
      {worker.certificationNames.length > 0 ? (
        ...
        <div className="flex flex-wrap gap-1">
          <span className="text-[9px] text-zinc-600">{t('companyWorkerSearch.noCertifications', 'No certifications')}</span>
        </div>
      )}
      ...
      >
        ...
        {t('companyWorkerSearch.viewProfile', 'View Profile')}
      </button>
    </div>
  );
}

function getAvailabilityBadge(worker: WorkerWithCounts, t: (key: string, fallback?: string) => string): { label: string; className: string } | null {
  if (worker.availability_status === 'available' || worker.employment_status === 'unemployed') {
    return { label: t('availability.availableNow', 'Available Now'), className: 'bg-green-950/50 border border-green-800/30 text-green-300' };
  }
  if (worker.availability_status === 'open_to_offers') {
    return { label: t('availability.openToOffers', 'Open to Offers'), className: 'bg-amber-950/50 border border-amber-800/30 text-amber-300' };
  }
  if (worker.availability_status === 'not_available') {
    return { label: t('availability.notAvailable', 'Not Available'), className: 'bg-zinc-800/50 border border-zinc-700/30 text-zinc-400' };
  }
  if (worker.employment_status === 'employed') {
    return { label: t('availability.employed', 'Employed'), className: 'bg-zinc-800/50 border border-zinc-700/30 text-zinc-400' };
  }
  return null;
}
