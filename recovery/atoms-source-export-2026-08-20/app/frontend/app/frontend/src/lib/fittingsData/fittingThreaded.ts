...
/**
 * ASME B16.11 — Threaded (NPT) Fittings (2000# / 3000# / 6000#)
 * Primary data: center-to-end or overall length for prefab.
 */

interface ThreadedFittingRaw {
  type: 'Elbow 90°' | 'Elbow 45°' | 'Tee' | 'Coupling' | 'Half Coupling' | 'Bushing' | 'Nipple';
  nps: string;
  class_rating: number;
  centerToEnd?: number; // mm
  length?: number;      // mm
}

const RAW: ThreadedFittingRaw[] = [
  // ─── 3000# Elbows 90° ───
  ...
  // ─── Close Nipples (common) ───
  { type: 'Nipple', nps: '1/2"',   class_rating: 3000, length: 38 },
  { type: 'Nipple', nps: '3/4"',   class_rating: 3000, length: 44 },
  { type: 'Nipple', nps: '1"',     class_rating: 3000, length: 51 },
  { type: 'Nipple', nps: '1-1/2"', class_rating: 3000, length: 64 },
  { type: 'Nipple', nps: '2"',     class_rating: 3000, length: 76 },
];

export const THREADED_FITTING_ENTRIES: FittingEntry[] = RAW.map((f) => ({
  category: 'fitting-threaded',
...
export const THREADED_FITTING_TYPES = ['Elbow 90°', 'Elbow 45°', 'Tee', 'Coupling', 'Nipple'] as const;
