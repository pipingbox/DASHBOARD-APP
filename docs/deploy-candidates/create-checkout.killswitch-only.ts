...
Deno.serve(async (req) => {
  ...
  // Monetization kill switch (Stream A containment, PO GO 2026-09-26 §7).
  ...
  // independently of what the app_stripe_prices catalog contains.
  // INLINE COPY of ../_shared/monetization.ts isMonetizationEnabled() so this
  // single-file candidate deploys without pulling any pending marketplace
  // backend (consent-evidence 5607211 / tax engine 5e44a53 are NOT included).
  if (Deno.env.get("MONETIZATION_ENABLED") !== "true") {
