...
// PD-COMPANY / PB-DRIFT-001: CompanyVerificationGate feature flag.
// The `company_verified` / `company_status` columns do NOT exist yet in
// TABLES.profiles (see brain/03-ENGINEERING/COMPANY-VERIFICATION-SCHEMA.md).
// Keep this OFF until that schema is deployed to Supabase. Flip to `true`
// only after the columns + RLS policies documented there are live.
// This must stay a simple boolean — no config system, no remote flags.
export const COMPANY_VERIFICATION_ENABLED = false;
...
export function CompanyVerificationGate({ children, allowAdmin = true }: CompanyVerificationGateProps) {
  ...
  const [state, setState] = useState<CompanyVerificationState>({
    ...
    company_status: null,
    loading: COMPANY_VERIFICATION_ENABLED,
  });

  useEffect(() => {
    // Feature flag OFF: let every company through (current production behavior)
    // but warn loudly so we notice any privileged-route usage while the real
    // verification schema/flow is not deployed yet.
    if (!COMPANY_VERIFICATION_ENABLED) {
      console.warn(
        '[CompanyVerificationGate] COMPANY_VERIFICATION_ENABLED=false — gate is bypassed. ' +
        'Company privileges are NOT enforced. See brain/03-ENGINEERING/COMPANY-VERIFICATION-SCHEMA.md.'
      );
      setState({ company_verified: true, company_status: null, loading: false });
      return;
    }
